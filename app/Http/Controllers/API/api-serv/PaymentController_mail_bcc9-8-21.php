    ...
    public function responseback(Request $request)
    {
      ...
      $timezones = $this->ckregion( $region, $country );
        ...
        if($all['channel_response_code']=="00" && !empty($check_is_recurring)  ){
          ...
          curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://netkasystem.com/itsmstore/api/upgrade-pay.php',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
          ...
          //  $scripts = 'aws_provisioning/AWS_db_provisioning -i '. $list[0] .' -n '. $list[2] . ' -p '. $list[1] . ' -t ' . $list[3] .' -e ' .$list[4].' -u '.$newpassword.' -tz '.$timezones;
            ...
            if($httpcode=="200"){
              ...
              Mail::send(array('html' => 'mail'), $data, function($message) use ($to_name, $to_email) {
                  ...
                  $message->bcc('atnatkasystem@gmail.com');
                  $message->subject('Your NetkaQuartz Service Desk X on Cloud is ready to use.');
                  $message->from('nsdx.saas@netkasystem.com','Your NSDX SaaS Ready');
              });
            }
        }else{
            // for recurring only
            ...
            curl_setopt_array($curl, array(
              CURLOPT_URL => 'https://netkasystem.com/itsmstore/api/callinvoice.php',//https://netkasystem.com/itsmstore/api/
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => '',
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 0,
...
//var_dump($datapost);exit;
        ...
        //curl send email receive upgrade plan
           ...
           $curl1 = curl_init();
            ...
            curl_setopt_array($curl1, array(
              CURLOPT_URL => 'https://netkasystem.com/itsmstore/api/receiveupgrade.php',//https://netkasystem.com/itsmstore/api/
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => '',
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 0,
   ...
   /*================  inquiry 2c2p ===================*/
    public function inquiry(Request $request){
      ...
      $secretKey = "DF6B6D630191EBAB3BB0DF6E2A35DCB11B8F7D7468E815E676570D86FE682833"; //env('SECRET_KEY2C2P'); //Get SecretKey from 2C2P PGW Dashboard
      //production key
      ...
      $payload = $this->encrypt2c2p($xml,"/var/www/html/newpayment/public/keys/cert.crt"); //Encrypt payload
      //Log 
      ...
      $response =  $this->decrypt2c2p($response,"/var/www/html/newpayment/public/keys/cert.crt","/var/www/html/newpayment/public/keys/private.pem","2c2p");
