...
class PaymentController extends Controller
{
    ...
    public function responseback(Request $request)
    {
      ...
      //type recurring payment plan
      $typee = $all['user_defined_1'];
      ...
      $country = strtoupper( trim($list[6]) );
        ...
        if( is_null($timezones) ){
          $scripts = 'aws_provisioning/AWS_db_provisioning -i '. $list[0] .' -n '. $list[2] . ' -p '. $list[1] . ' -t ' . $list[3] .' -e ' .$list[4].' -u '.$newpassword.' -rp '.$typee.' -f 0';
        }else{
          $scripts = 'aws_provisioning/AWS_db_provisioning -i '. $list[0] .' -n '. $list[2] . ' -p '. $list[1] . ' -t ' . $list[3] .' -e ' .$list[4].' -u '.$newpassword.' -tz '.$timezones.' -rp '.$typee.' -f 0';
        }
        //set exprice 
        switch ($list[3]){
            case "DEMO"://free 30 day
                $expire = 'Free Trial to use until: '.date('Y-m-d',strtotime($date1 . "+30 days"));
            break;
        ...
        if($all['channel_response_code']=="00" && !empty($check_is_recurring) ){
          ...
          // if( !empty($all['recurring_unique_id']) ){
          //   //update expired provisioning
          //   //update expired subscript fontend and send email pdf invoice
          //   $invoice = $all['invoice_no']; //invoice number
          //   $order_id = $all['order_id']; //order id
          ...
          // }else{
            ...
            $urlckcurl = 'https://'.$list[0].'.itsmnetka.com/shared/checkempty';//'http://'.$username.'.itsmnetka.com/Login';
            $i=10;
            $k=0;
            $httpcode = "";
            while($k<=$i){
          ...
          //log เฉพาะ recurring
            ...
            //type recurring payment plan
            //$typee = $all['user_defined_1'];
            //$check_is_recurring = $all['recurring_unique_id']; 
            $recuringorder_id = $all['order_id']; //order id
            $username = $list[0]; //username
            $emailcus = $list[4]; //email customer
            $invoice = $all['invoice_no']; //invoice number find 
            ...
            //script 
            $scripts = 'aws_provisioning/AWS_db_provisioning -i '. $list[0] .' -n '. $list[2] . ' -p '. $list[1] . ' -t ' . $list[3] .' -e ' .$list[4].' -u null -tz '.$timezones.' -rp '.$typee.' -f 1';
            ...
            curl_setopt_array($curl, array(
              CURLOPT_URL => 'localhost:8000/itsmstore4/api/callinvoice.php',
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => '',
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 0,
...
//charge for free
    public function forfree(Request $request){
        ...
        $timezones = $this->ckregion( $region, $country );
        if( is_null($timezones) ){
          $scripts = 'aws_provisioning/AWS_db_provisioning -i '. $username .' -n '. $agent . ' -p '. $package . ' -t ' . $type .' -e ' .$email.' -u '.$newpassword.' -rp 30 -f 0';
        }else{
          $scripts = 'aws_provisioning/AWS_db_provisioning -i '. $username .' -n '. $agent . ' -p '. $package . ' -t ' . $type .' -e ' .$email.' -u '.$newpassword.' -tz '.$timezones.' -rp 30 -f 0' ;
        }
