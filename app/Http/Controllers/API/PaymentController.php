...
class PaymentController extends Controller
{
    public function cksendmail(Request $request)
    {
        ...
        Mail::send(array('html' => 'mail'), $data, function($message) use ($to_name, $to_email) {
              ...
              $message->bcc('wattawadee@netkasystem.com');
              $message->subject('Your NetkaQuartz Service Desk X on Cloud is ready to use.');
              $message->from('itsm@netkasystem.com','Your NSDX SaaS Ready'); //nsdx.saas@netkasystem.com
        });
    }
    ...
    public function responseback(Request $request)
    {
      ...
      // list parameter response code==00 only
      if( $all['channel_response_code']=="00" ){
        ...
        /* ======================= case check first payment recurring 
          * or auto recurring cut amount per day,month,year ============== */

        if($all['channel_response_code']=="00" && !empty($check_is_recurring)  ){ //payment recurring
          ...
          curl_setopt_array($curl, array(
            CURLOPT_URL => 'http://localhost:8000/itsmstore4/api/upgrade-pay.php',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
          ...
          //  $scripts = 'aws_provisioning/AWS_db_provisioning -i '. $list[0] .' -n '. $list[2] . ' -p '. $list[1] . ' -t ' . $list[3] .' -e ' .$list[4].' -u '.$newpassword.' -tz '.$timezones;
            ...
            $urlckcurl = 'http://'.$list[0].'.itsmnetka.com/shared/checkempty';//'http://'.$username.'.itsmnetka.com/Login';
            $i=10;
            $k=0;
            $httpcode = "";
            while($k<=$i){
            ...
            if($httpcode=="200"){
              ...
              $data = array('name'=> $list[0],
                           "servicelink" => "https://".$list[0].".itsmnetka.com",
                            "agent" => $list[2],
                            "package" => $planpackage,
                            "expire" =>$expireformat,
                            "genpass" => $newpassword,
        ...
        }else if( $all['channel_response_code']=="00" && empty($check_is_recurring) ){
            // for auto recurring only next day,mont,year update -rp : days 
            //log เฉพาะ recurring
            ...
            curl_setopt_array($curl, array(
              CURLOPT_URL => 'localhost:8000/itsmstore4/api/callinvoice.php',//https://netkasystem.com/itsmstore/api/
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => '',
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 0,
        ...
        /*==== check case reject response code !=00  16-08-21*/
        //if( $all['channel_response_code']!="00" && empty($check_is_recurring) ){ //is recurring response only
          //log response reject
          Log::channel('recurring_reject')->info('reject_info',['data' => $all]);

          $recuringorder_id = $all['order_id'];
          $flist = substr($recuringorder_id, 0,8);
          $selist = substr($recuringorder_id, 8,-5);
          $thlist = substr($recuringorder_id, -5);
          $order_id = $selist;
          $dataupdatestatus = 'type=reject&order_id='.$order_id.'&status_order=on-hold&status_subscription=suspend';

          //curl call api update status to subspened and send email to customer
            $curl = curl_init();

            curl_setopt_array($curl, array(
              CURLOPT_URL => 'localhost:8000/itsmstore4/api/updatestatus.php',//https://netkasystem.com/itsmstore/api/
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => '',
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 0,
              ...
              CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
              CURLOPT_CUSTOMREQUEST => 'POST',
              CURLOPT_POSTFIELDS => $dataupdatestatus,
              CURLOPT_HTTPHEADER => array(
                'Content-Type: application/x-www-form-urlencoded'
              ),
            ));

            $responsereject = curl_exec($curl);

            curl_close($curl);

            //log response updatestatus
            Log::channel('recurring_reject_upstatus')->info('reject_info',['data' => $responsereject]);
            echo $responsereject;
        }
	}
...
//charge for free
    public function forfree(Request $request){
       ...
       // //  //log call script
        ...
        $urlckcurl = 'https://'.$username.'.itsmnetka.com/shared/checkempty';//'http://'.$username.'.itsmnetka.com/Login';
        ...
        if($httpcode=="200"){
            ...
            $data = array('name'=> $username,
                               "servicelink" => "https://".$username.".itsmnetka.com",
                                "agent" => $agent,
                                "package" => $planpackage,
                                "expire" => $expireformat,
                                "genpass" => $newpassword,
        ...
        echo json_encode( array("mscode"=>"successful","message"=>"successful") ); 
    }
    ...
    */
    ...
    public function responsebackupgrades(Request $request){
      ...
      $date_payment = substr($find_oldorder[0], -8); //date pay U476810215620210702O4769
      $old_order = $find_oldorder[1]; //old order id 
      $cut_fix = substr($find_oldorder[0], 1); //cut Fix U out
      // $order_subscript = substr($cut_fix, 0,-8); //order is subscription
      // change for upgrade more time use under comment
      ...
      $amount_difference = $all['amount'];
// $datapost = 'old_order='.$old_order.'&subscription_order='.$order_subscript.'&cus_email='.$customer_email.'&detail_upgrade='.$detail_upgrade.'&paydiff='.$amount_difference.'&order_id='.$response_orderid;
// var_dump($response_orderid,$date_payment,$order_subscript);
// exit;
      $today = date("Y-m-d");
      $date1 = str_replace('-', '/', $today);
      $newpassword = '';//$this->GenerateStrongPassword(); //16 digit
...
//var_dump($datapost);exit;
    ...
    public function inquiry(Request $request){
      ...
      $test_res = "<RecurringMaintenanceResponse><version>2.4</version><timeStamp>060721115227</timeStamp><respCode>00</respCode><respReason>Cancel Successful</respReason><recurringUniqueID>159393</recurringUniqueID><hashValue>3ABFD203DB3798618B17066951ED75B787174CF3F6AD1F85896B0439D02FFE5F</hashValue></RecurringMaintenanceResponse>";

      libxml_use_internal_errors(TRUE);
 
      $objXmlDocument = simplexml_load_string($test_res) or die("Error: Cannot create object");
      var_dump($objXmlDocument->respCode);
      exit;
      //Log
...
//       exit;
     ...
     // $pkcs7 = new pkcs7();
      ...
      $payload = $this->encrypt2c2p($xml,"http://localhost:8000/newpayment1/public/keys/demo2.crt"); //Encrypt payload
      //Log 
     ...
     // $http = new HTTP();
      ...
      $url = "https://demo2.2c2p.com/2C2PFrontend/PaymentActionV2/PaymentAction.aspx";
      ...
      $response =  $this->decrypt2c2p($response,"http://localhost:8000/newpayment1/public/keys/demo2.crt","http://localhost:8000/newpayment1/public/keys/demo2.pem","2c2p");
    ...
    /*=============== response case Re-Payment can change card number 30-08-21 ============================*/
    public function responsebackrepay(Request $request){
      $all = $request->all();
      Log::channel('repayment')->info('info-repay',['data' => $all]);
      // exit;
      ...
      $response_orderid = $all['order_id'];// order id
      $find_oldorder = explode('O', $all['order_id']);
      $date_payment = substr($find_oldorder[0], -8); //date pay RE476810215620210702O4769
      $old_order = $find_oldorder[1]; //old order id 
      $cut_fix = substr($find_oldorder[0], 2); //cut Fix RE out
      $order_subscript = substr(substr($cut_fix, 0,-8) ,0,-6); //order is subscription
      $sitename = $list[0]; //user sitename
      $customer_email =  $list[4]; //customer email
      $amount_difference = $all['amount'];
      ...
      // $scriptoutput =  "<pre>$output</pre>";
      // //log call script
      // Log::channel('scriptrepayment')->info('script',['data' => $scripts,'output' => $scriptoutput]);

      /*====cancel recurring old and update status form suspending to active ====*/

      $datapost = 'type=repay&order_id='.$old_order.'&status_order=completed&status_subscription=active';

      // $curl = curl_init();

      // curl_setopt_array($curl, array(
      //   CURLOPT_URL => 'http://localhost:8000/itsmstore4/api/updatestatus.php',//https://netkasystem.com/itsmstore/api/
      //   CURLOPT_RETURNTRANSFER => true,
      //   CURLOPT_ENCODING => '',
      //   CURLOPT_MAXREDIRS => 10,
      //   CURLOPT_TIMEOUT => 0,
      ...
      // curl_close($curl);
      //Log response updatestatus
      // Log::channel('repayment_updatestatus')->info('updatestatus-repay',['re-data' => $datapost,'data' => $responsereject]);    

      //insert log_recurring_res
       /*====save log recurring new====*/ 
      $datapost_savelog = json_encode( array( 'type_pay'=> 'recurring',
                    'old_order' => $old_order,
                    'subscription_order' => $order_subscript,
                    'cus_email' => $customer_email,
                    'detail_upgrade' => 'repayment recurring',
                    'res_recurring' => $all
                  ) );
      ...
      //   CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
      //   CURLOPT_CUSTOMREQUEST => 'POST',
      //   CURLOPT_POSTFIELDS =>$datapost_savelog,
      //   CURLOPT_HTTPHEADER => array(
      //     'Content-Type: application/json'
      //   ),
      // ));
      ...
      // curl_close($curl2);
      //send email callinvoice
      /*====send email callinvoice====*/
      $datapost_receive = 'invoice_no='.$response_orderid.'&email='.$customer_email.'&username='.$sitename.'&order_id='.$old_order;
      $curl3 = curl_init();

      curl_setopt_array($curl3, array(
        CURLOPT_URL => 'http://localhost:8000/itsmstore4/api/callinvoice.php',//'https://staging2.netkasystem.com/itsmstore/api/callinvoice.php',//https://netkasystem.com/itsmstore/api/
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
        ...
        CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
        CURLOPT_CUSTOMREQUEST => 'POST',
        CURLOPT_POSTFIELDS => $datapost_receive,
        CURLOPT_HTTPHEADER => array(
          'Content-Type: application/x-www-form-urlencoded'
        ),
      ));

      $response_callreceive = curl_exec($curl3);

      curl_close($curl3);
      //Log response send email callinvoice
      // Log::channel('repayment_callreceive')->info('ucallreceive-repay',['re-data' => $datapost_receive,'data' => $response_callreceive]);
      echo $response_callreceive;
      //exit;
    ...
    $timezones = $this->ckregion( $region, $country );
      ...
      switch ($package) {
        ...
        //  $scripts = 'aws_provisioning/AWS_db_provisioning -i '. $list[0] .' -n '. $list[2] . ' -p '. $list[1] . ' -t ' . $list[3] .' -e ' .$list[4].' -u '.$newpassword.' -tz '.$timezones;
          ...
          $urlckcurl = 'https://'.$sitename.'.itsmnetka.com/shared/checkempty';//'http://'.$username.'.itsmnetka.com/Login';
          $i=10;
          $k=0;
          $httpcode = "";
          while($k<=$i){
          ...
          if($httpcode=="200"){
            ...
            $data = array('name'=> $sitename,
                         "servicelink" => "https://".$sitename.".itsmnetka.com",
                          "agent" => $agent,
                          "package" => $planpackage,
                          "expire" =>$expireformat,
                          "genpass" => $newpassword,
